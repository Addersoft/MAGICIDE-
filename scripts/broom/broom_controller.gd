...
@export var damp: float = 14.0
@export var retrograde: float = 1.6
@export var altitude_lock: float = 36.0
@export var visual_roll_degrees: float = 12.0
...
func integrate(delta: float, current: Vector3, yaw_basis: Basis, axis: Vector2, vertical: float, knock_h: Vector3) -> Vector3:
	...
	var next := Vector3(
		lerpf(current.x, desired.x, alpha),
		0.0,
		lerpf(current.z, desired.z, alpha)
	)
...
func update_visual(delta: float, axis: Vector2) -> void:
	if not mounted:
		return
	visual.rotation.y = view.rotation.y
	var target_roll: float = -axis.x * deg_to_rad(visual_roll_degrees)
